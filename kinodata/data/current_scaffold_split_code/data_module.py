...
from kinodata.data.grouped_split import KinodataKFoldSplit, joint_scaffold_kfold_split, report_scaffold_distribution
...
# --- NEW: synchronized scaffold splits across activity & pose ---
    if config.split_type == "scaffold-k-fold" and not config.data_split:
        joint_splits = joint_scaffold_kfold_split(
        activity_ds=activity_ds,
        pose_ds=pose_ds,
        k=config.k_fold,
        inner_val_frac=0.5,  # keep your existing 50/50 val/test inside fold
        max_samples_per_scaffold=getattr(config, "max_samples_per_scaffold", None),
        seed=getattr(config, "seed", 0),
        )
        split_act, split_pose = joint_splits[config.split_index]

        

        act_scaffolds = np.array([d.scaffold for d in activity_ds])
        pose_scaffolds = np.array([d.scaffold for d in pose_ds])

        for fold_id, (split_act, split_pose) in enumerate(joint_splits):
            report_scaffold_distribution(fold_id, "activity", split_act, act_scaffolds, tag="scaffold", topn=5)
            report_scaffold_distribution(fold_id, "pose",     split_pose, pose_scaffolds, tag="scaffold", topn=5)
