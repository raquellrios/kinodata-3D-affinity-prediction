...
def group_k_fold_split(
    ...
    all_scaffolds = group_index.copy()
    filtered_indices = np.arange(len(all_scaffolds))
    ...
    if max_samples_per_scaffold is not None:
        ...
        group_index = all_scaffolds[filtered_indices]
    ...
    for train_idx, test_idx in generator:
        ...
        #test_scaffolds = np.unique(group_index[test_global])
        held_scaffolds = all_scaffolds[test_global]
        val_scaffolds, test_scaffolds = _split_random(np.unique(held_scaffolds), 0.5)
        #val_scaffolds, test_scaffolds = _split_random(test_scaffolds, 0.5)

        #val_idx   = [i for i in test_global if group_index[i] in val_scaffolds]
        #test_idx  = [i for i in test_global if group_index[i] in test_scaffolds]

        val_idx   = [i for i in test_global if all_scaffolds[i] in val_scaffolds]
        test_idx  = [i for i in test_global if all_scaffolds[i] in test_scaffolds]
...
def unified_scaffold_splits(
    scaffolds_a: np.ndarray,
    scaffolds_b: np.ndarray,
    split_type: str,
    k: int,
    max_samples_per_scaffold: Optional[int] = None,
) -> List[Dict[str, Split]]:
    """
    Returns a list of length-k of dicts {'activity': Split, 'pose': Split},
    where scaffolds for both datasets have been split jointly using GroupKFold.
    """
        # 1) ensure input is array of scaffold strings
    def _ensure_array(x):
        if isinstance(x, np.ndarray):
            return x
        # fallback: assume it's an iterable of data objects
        try:
            return np.array([d.scaffold for d in x])
        except Exception:
            raise ValueError("unified_scaffold_splits requires arrays or datasets with .scaffold attribute")

    arr_a = _ensure_array(scaffolds_a)
    arr_b = _ensure_array(scaffolds_b)
    # concatenate the two scaffold arrays
    scaff_global = np.concatenate([arr_a, arr_b])

    # 2) generate global splits via the existing group_k_fold_split helper
    splits_global = group_k_fold_split(
        group_index=scaff_global,
        k=k,
        max_samples_per_scaffold=max_samples_per_scaffold,
    )

    # Optional: print stats for the combined splits
    _print_scaffold_stats_per_fold(scaff_global, splits_global, tag="unified-scaffold")

    # 3) map global indices back to per-dataset splits
    n_a = len(scaffolds_a)
    n_b = len(scaffolds_b)
    unified = []
    for g in splits_global:
        # split out activity indices (global index < n_a)
        act_train = [i for i in g.train_split if i < n_a]
        act_val   = [i for i in g.val_split   if i < n_a]
        act_test  = [i for i in g.test_split  if i < n_a]

        # split out pose indices (global index >= n_a)
        pose_train = [i - n_a for i in g.train_split if i >= n_a]
        pose_val   = [i - n_a for i in g.val_split   if i >= n_a]
        pose_test  = [i - n_a for i in g.test_split  if i >= n_a]

        unified.append({
            'activity': Split(act_train, act_val, act_test),
            'pose': Split(pose_train, pose_val, pose_test),
        })
...
class KinodataKFoldSplit:
    ...
    def _split(self, dataset):
        if self.split_type == "scaffold-k-fold":
            ...
            splits = group_k_fold_split(group_index=scaffolds, k=self.k, max_samples_per_scaffold=self.max_samples_per_scaffold) #max_samples_per_scaffold=1000)
