...
class RegressionModel(pl.LightningModule):
    ...
    def training_step(self, batch, *args) -> Tensor:    #for loop over the bacthes 
        ...
        self.log("train/loss_activity", loss_activity.mean(), batch_size=pred_activity.size(0), on_epoch=True, on_step=True)
    
        # Forward pass for pose batch
        pred_pose = self.forward(pose_batch)
        loss_pose = self.compute_loss_pose(pred_pose, pose_batch)
        self.log("train/loss_pose", loss_pose.mean(), batch_size=pred_pose.size(0), on_epoch=True, on_step=True)
