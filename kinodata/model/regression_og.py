...
class RegressionModel(pl.LightningModule):
    ...
    def training_step(self, batch, *args) -> Tensor:
        ...
        pred = self.forward(activity_batch).view(-1, 1)
        print("training")
        print("pred")
        print(pred)
        print("target")
        print(activity_batch.y)
        loss = self.criterion(pred, activity_batch.y.view(-1, 1))
        self.log("train/loss", loss, batch_size=n_act, on_epoch=True)
        return loss

    def validation_step(self, batch, *args, key: str = "val"):
        activity_batch, pose_batch = batch  # batch is a tuple from the DataLoader
        n_act, n_pose = activity_batch.num_graphs, pose_batch.num_graphs
        pred = self.forward(activity_batch).flatten()
        val_mae = (pred - activity_batch.y).abs().mean()
        print("validation")
        print("pred")
        print(pred)
        print("target")
        print(activity_batch.y)
        self.log(f"{key}/mae", val_mae, batch_size=n_act, on_epoch=True)
        return {
            f"{key}/mae": val_mae,
            "pred": pred,
            "target": activity_batch.y,
            "ident": activity_batch.ident,
        }
    ...
    def predict_step(self, batch, *args):
        ...
        return {
            "pred": pred,
            "target": activity_batch.y.flatten(),
            "chembl_activity_id": activity_batch.chembl_activity_id,
        }
