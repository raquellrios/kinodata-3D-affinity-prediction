...
class CombinedDataset(Dataset):
    ...
    def __len__(self):
        return (
            self.min_len
        )  # Adjust this if you want equal-size batches or oversampling
...
class CombinedDataModule(pl.LightningDataModule):
    ...
    def setup(self, stage=None):
        ...
        self.train_dataset = CombinedDataset(
            self.datamodule1.train_dataset, self.datamodule2.train_dataset
        )

        # Combine validation datasets
        self.val_dataset = CombinedDataset(
            self.datamodule1.val_dataset, self.datamodule2.val_dataset
        )

        # Combine test datasets
        self.test_dataset = CombinedDataset(
            self.datamodule1.test_dataset, self.datamodule2.test_dataset
        )
